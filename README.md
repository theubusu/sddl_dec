...
`[OPTIONS]` - Can be:  
`-d` - Debug mode - print debug information about extraction process.  
...
- A bunch of XXX.TXT files which store information about the update (These are skipped by default and printed to output instead. You can prevent that with the `-k` option.)
